	...
	public function HtmlShowOptionsPage() {
		...
		if(!empty($_REQUEST["sm_rebuild"])) { //Pressed Button: Rebuild Sitemap
			...
			if(isset($_GET["sm_do_debug"]) && $_GET["sm_do_debug"]=="true") {
				...
				echo '<div class="wrap">';
				echo '<h2>' .  __('BH XML Sitemap Generator for WordPress', 'sitemap') .  " " . $this->sg->GetVersion(). '</h2>';
				echo '<p>This is the debug mode of the BH XML Sitemap Generator. It will show all PHP notices and warnings as well as the internal logs, messages and configuration.</p>';
...
HTML;
		...
		<div class="wrap" id="sm_div">
			<form method="post" action="<?php echo $this->sg->GetBackLink() ?>">
				<h2><?php _e('BH XML Sitemap Generator for WordPress', 'sitemap'); echo " " . $this->sg->GetVersion() ?> </h2>
				<?php
				...
				?>
					...
					<ul>
						<li>
							...
							<small><?php echo str_replace("%s",$this->sg->GetRedirectLink('sitemap-gt'),__('No registration required, but you can join the <a href="%s">Google Webmaster Tools</a> to check crawling statistics.','sitemap')); ?></small>
						</li>
						<li>
							...
							<small><?php echo str_replace("%s",$this->sg->GetRedirectLink('sitemap-bt'),__('No registration required, but you can join the <a href="%s">Bing Webmaster Tools</a> to check crawling statistics.','sitemap')); ?></small>
						</li>
						<li>
					...
					<?php if(is_super_admin()): ?>
						...
						<?php
							...
							_e("Enter the URL to the page. Examples: http://www.domain.com/index.html or www.domain.com/home ",'sitemap');
							echo "</li><li>";
